...
async function resizeImage(imgSrc, imgDst, width, height = undefined) {
...
export async function handler(event) {
    const filesProcessed = event.Records.map(async (record) => {
        ...
        const inputData = await s3.send(getObjectCommand);

        // Resize the file
        const tempFile = `${os.tmpdir()}/${crypto.randomUUID()}.png`;
        await resizeImage(inputData.Body, tempFile, 150);

        // Read the resized file
        const resizedData = await fs.readFile(tempFile);

        // Upload the new file to s3
        const targetFilename = `${filename.substring(0, filename.lastIndexOf("."))}-small.png`;
        params = {
            Bucket: `${bucket}-dest`,
            Key: targetFilename,
            Body: resizedData,
